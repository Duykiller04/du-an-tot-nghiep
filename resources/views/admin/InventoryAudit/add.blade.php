                ...
                <div class="card p-3">
                    <div class="card-body">
                        <form action="{{ route('admin.inventoryaudit.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="title">Tiêu đề phiếu kiểm<span
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="check_date">Ngày kiểm<span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="check_by">Người kiểm<span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('check_by') is-invalid @enderror"
                                    id="check_by" name="check_by" value="{{ old('check_by') }}">
                                @error('check_by')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div id="details-container">
                                @for ($i = 0; $i < max(count(old('details', [1])), session('detail_count', 1)); $i++)
                                    <div class="row mb-3 detail-row">
                                        <div class="col-lg-3">
                                            <label class="form-label" for="medicine_id">Loại thuốc<span
                                                    class="text-danger">*</span></label>
                                            <select
                                                class="form-control @error('details.' . $i . '.medicine_id') is-invalid @enderror"
                                                name="details[{{ $i }}][medicine_id]">
                                                <option value="">Chọn thuốc</option>
                                                @foreach ($medicines as $medicine)
                                                    <option value="{{ $medicine->id }}"
                                                        {{ old('details.' . $i . '.medicine_id') == $medicine->id ? 'selected' : '' }}>
                                                        {{ $medicine->medicine_code }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-lg-2">
                                            <label class="form-label" for="expected_quantity">Số lượng mong muốn<span
                                                    class="text-danger">*</span></label>
                                            <input type="number"
                                                class="form-control @error('details.' . $i . '.expected_quantity') is-invalid @enderror"
                                                name="details[{{ $i }}][expected_quantity]"
                                                value="{{ old('details.' . $i . '.expected_quantity') }}">
                                            @error('details.' . $i . '.expected_quantity')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label" for="actual_quantity">Số lượng thực tế<span
                                                    class="text-danger">*</span></label>
                                            <input type="number"
                                                class="form-control @error('details.' . $i . '.actual_quantity') is-invalid @enderror"
                                                name="details[{{ $i }}][actual_quantity]"
                                                value="{{ old('details.' . $i . '.actual_quantity') }}">
                                            @error('details.' . $i . '.actual_quantity')
                                                <div class="text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-lg-2">
                                            <label class="form-label" for="difference">Chênh lệch<span
                                                    class="text-danger">*</span></label>
                                            <input type="number"
                                                class="form-control @error('details.' . $i . '.difference') is-invalid @enderror"
                                                name="details[{{ $i }}][difference]"
                                                value="{{ old('details.' . $i . '.difference') }}">
                                        ...
                                        <div class="col-lg-2">
                                            <label class="form-label" for="remarks">Ghi chú</label>
                                            <input type="text"
                                                class="form-control @error('details.' . $i . '.remarks') is-invalid @enderror"
                                                name="details[{{ $i }}][remarks]"
                                                value="{{ old('details.' . $i . '.remarks') }}">
                            ...
                            <div>
                                <button id="btn-add-detail" type="button" class="btn btn-primary">Thêm chi tiết
                                    +</button>
                            </div>

                            <div class="text-end mb-4">

                                <a href="{{ route('admin.inventoryaudit.index') }}" class="btn btn-warning w-sm">Quay
                                    lại</a>
